...
export const removeEmptyValue = <O extends RecursionObject<string | number | boolean | undefined | null | Date>>(
  obj: O,
  filter: FilterValue[]
): O => {
  ...
  for (const key of keys) {
    ...
    switch (typeof value) {
      case 'object': {
        ...
        } else if (Object.keys(value).length === 0 && filter.includes(FilterValue.EMPTY_OBJECT)) {
          delete object[key];
        } else {
          // @ts-ignore
