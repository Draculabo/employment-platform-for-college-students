...
const registerRouters = (version: `v${number}`) => (fastifyServer: FastifyInstance, controllers: Array<(server: Server) => void>) => {
    ...
    return <S>(
      ...
      fastifyServer[method](
        `/${version}/${path}`,
        {
          ...
          // warning
          schema: config.schema as FastifySchema,
        },
...
interface R<O> {
  <S>(
    path: string,
    //@ts-ignore
    handler: (req: FastifyRequestTypebox<S>, reply: FastifyReply) => Promise<O extends false ? void : Response>,
    config: {
      auth?: boolean;
      schema: S;
      autoHandle?: O;
