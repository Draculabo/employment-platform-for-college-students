...
class RedisService {
  ...
  public async hmget(key: string, field: string | string[]): Promise<string | null | Array<string | null>> {
    if (typeof field === 'string') {
      return (await this.client.hmget(key, field))[0];
    }
    // new api,  maybe occur error
    return await this.client.hmget(key, ...field);
  }

  public async scan(match: string, count = 100, totalCount: boolean | number = false): Promise<string[]> {
  ...
  public async scanCallback(func: (key: string[]) => Promise<void>, match: string, count = 100): Promise<void> {
    const stream = this.client.scanStream({
      match,
      count,
    });
