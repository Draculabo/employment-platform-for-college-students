import lodash, { cloneDeep, curry } from 'lodash';
...
export class Logger<C extends LoggerContext> {
  ...
  constructor(private readonly label: string, context: Partial<C>, private readonly plugins: LoggerAbstractPlugin<C>[]) {
  ...
  private logFormat(level: LoggerLevel, message: string, meta: Partial<C>): LoggerFormat<Partial<C>> {
    return {
      timestamp: dayjs().format("yyyy-MM-dd'T'HH:mm:ss.SSSxxx"),
      label: this.label,
      level,
      message,
      payload: removeEmptyValue(
        ...
        [FilterValue.UNDEFINED, FilterValue.EMPTY_OBJECT, FilterValue.NULL, FilterValue.NAN]
      ),
    };
  }
}
