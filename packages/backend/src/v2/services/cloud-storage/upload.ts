...
import { FileConvertStep, FileResourceType, whiteboardResourceType } from '../../../model/cloudStorage/Constants';
...
export class CloudStorageUploadService {
  ...
  public async start(config: CloudStorageUploadStartConfig): Promise<CloudStorageUploadStartReturn> {
    ...
    await new CloudStorageDirectoryService(this.ids, this.DBTransaction, this.userUUID).assertExists(targetDirectoryPath);

    const fileResourceType = new CloudStorageFileService(this.ids, this.DBTransaction, this.userUUID).getFileResourceTypeByFileName(
      fileName
    );
  ...
  public async finish(config: CloudStorageUploadFinishConfig): Promise<void> {
    ...
    const { fileName, fileSize, targetDirectoryPath, fileResourceType } = await this.getFileInfoByRedis(fileUUID);
  ...
  public async assertConcurrentLimit(): Promise<void> {
    const uploadingFiles = await RedisService.scan(RedisKey.cloudStorageFileInfo(this.userUUID, '*'), CloudStorage.concurrent + 1, true);
  ...
  public async getTotalUsageByUpdated(currentFileSize: number): Promise<number> {
    const { total_usage } = await cloudStorageConfigsDAO.findOne(this.DBTransaction, 'total_usage', {
      user_uuid: this.userUUID,
    });
  ...
  public static generateOSSFilePath = (fileName: string, fileUUID: string): string => {
    ...
    return `${CloudStorage.prefixPath}/${datePath}/${fileUUID}/${fileUUID}${path.extname(fileName)}`;
  };

  public async getFileInfoByRedis(fileUUID: string): Promise<GetFileInfoByRedisReturn> {
    const fileInfo = await RedisService.hmget(RedisKey.cloudStorageFileInfo(this.userUUID, fileUUID), [
      'fileName',
      'fileSize',
      'targetDirectoryPath',
      'fileResourceType',
