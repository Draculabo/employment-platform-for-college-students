...
export class DeveloperOAuthAuthorizeService {
  ...
  public async view(config: DeveloperOAuthAuthorizeAuthorizeConfig): Promise<DeveloperOAuthAuthorizeAuthorizeReturn> {
    ...
    const callbackURL = oauthInfo.callbacksURL.find((item) => config.redirectURI.startsWith(item));
    ...
    const developerOAuthUserSVC = new DeveloperOAuthUserService(this.ids, this.DBTransaction, this.userUUID);
  ...
  public async infoByClientID(clientID: string): Promise<DeveloperOAuthAuthorizeAuthorizeInfoByClientIDReturn> {
  ...
  public async generateAuthorizeCSRFToken(oauthUUID: string, token?: string): Promise<string> {
    ...
    await RedisService.set(RedisKey.oauthAuthorizeCSRFToken(oauthUUID, this.userUUID), csrfToken, 60 * 10);

    return csrfToken;
  }

  public async getAuthorizeCSRFToken(oauthUUID: string): Promise<string | null> {
    const csrfToken = await RedisService.get(RedisKey.oauthAuthorizeCSRFToken(oauthUUID, this.userUUID));
  ...
  public async saveOAuthAuthorizeScopes(oauthUUID: string, scopes: DeveloperOAuthScope[]): Promise<void> {
    await RedisService.set(RedisKey.oauthAuthorizeScopes(oauthUUID, this.userUUID), JSON.stringify(scopes), 60 * 10);
  }

  public async getOAuthAuthorizeScopes(oauthUUID: string): Promise<DeveloperOAuthScope[]> {
    const scopes = await RedisService.get(RedisKey.oauthAuthorizeScopes(oauthUUID, this.userUUID));
    if (!scopes) {
  ...
  public async generateOAuthToken(clientID: string): Promise<GenerateOAuthTokenReturn> {
    const [accessToken, refreshToken] = await Promise.all(Array.from({ length: 2 }, () => DeveloperOAuthAuthorizeService.nanoID()));
