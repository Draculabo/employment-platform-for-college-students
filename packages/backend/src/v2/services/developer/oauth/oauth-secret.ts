...
import { DeveloperOAuthSecretCreateReturn, DeveloperOAuthSecretInfoReturn } from './oauth-secret.type';
...
export class DeveloperOAuthSecretService {
  ...
  public async create(oauthUUID: string): Promise<DeveloperOAuthSecretCreateReturn> {
    ...
    const clientID = await new DeveloperOAuthInfoService(this.ids, this.DBTransaction, this.userUUID).findClientID(oauthUUID);
  ...
  public async assertIsOwner(secretUUID: string): Promise<void> {
    const { oauth_uuid: oauthUUID } = await oauthSecretsDAO.findOne(this.DBTransaction, 'oauth_uuid', {
      secret_uuid: secretUUID,
    });
    ...
    await new DeveloperOAuthInfoService(this.ids, this.DBTransaction, this.userUUID).assertIsOwner(oauthUUID);
  }
