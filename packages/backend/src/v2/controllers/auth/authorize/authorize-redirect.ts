...
export const oauth2AuthorizeRedirect = async (
  ...
  if (!csrfToken) {
    return reply.redirect(302, `${redirectURI}${sep}error=access_denied&error_description=csrf+token+expired&state=${req.body.state}`);
  }
  ...
  const [err2, scopes] = await errPairSync(developerOAuthAuthorizeSVC.getOAuthAuthorizeScopes(req.body.oauthUUID));
  if (err2) {
    return reply.redirect(302, `${redirectURI}${sep}error=invalid_scope&error_description=scope+is+expired&state=${req.body.state}`);
  }
