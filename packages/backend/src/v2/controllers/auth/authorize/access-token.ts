...
export const oauth2AccessToken = async (
  ...
  const [err, userUUID] = await errPairSync(DeveloperOAuthAuthorizeService.getAuthorizeCodePayload(req.body.code));
  if (err) {
    // @ts-ignore
  ...
  {
    const developerOAuthSecretSVC = new DeveloperOAuthSecretService(req.ids, req.DBTransaction, userUUID);
    const [err] = await errPairSync(developerOAuthSecretSVC.assertExist(req.body.clientID, req.body.clientSecret));
    if (err) {
      // @ts-ignore
      // eslint-disable-next-line @typescript-eslint/no-unsafe-call
  ...
  const developerOAuthAuthorizeSVC = new DeveloperOAuthAuthorizeService(req.ids, req.DBTransaction, userUUID);
  ...
  {
    const result = await RedisService.hmget(RedisKey.oauthAuthorizeTokenByUserUUID(req.body.clientID, userUUID), [
      'accessToken',
      'refreshToken',
    ]);
