...
export const userRename = async (req: FastifyRequestTypebox<typeof userRenameSchema>): Promise<Response> => {
  const complianceText = useOnceService('complianceText', req.ids);
  await complianceText.assertTextNormal(req.body.newUserName);

  await new UserUpdateService(req.ids, req.DBTransaction, req.userUUID).userName(req.body.newUserName);
