...
import { insertRoleSchema, roleListSchema, updateRoleSchema } from '@/v2/services/role/schema';
import RoleService from '@/v2/services/role/role';

export const updateRole = async (req: FastifyRequestTypebox<typeof updateRoleSchema>) => {
  await new RoleService(req.ids, req.DBTransaction, req.userUUID).updateRole(req.body);
  return successJSON({});
};
