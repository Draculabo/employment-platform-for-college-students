  ...
  return (transaction) => {
    ...
    return {
      ...
      insert: (data, flag) => {
        ...
        if (flag?.orIgnore) {
          return managerOrRepo.createQueryBuilder().insert().into(model).orIgnore().values(data).execute();
        }

        return managerOrRepo.createQueryBuilder().insert().into(model).values(data).execute();
      },
      update: (setData, where, order, limit) => {
        const result = managerOrRepo.createQueryBuilder().update(model).set(setData).where(noDelete(where));
      ...
      physicalDeletion: (where) => {
        return managerOrRepo.createQueryBuilder().delete().from(model).where(where).execute();
      },
    };
  };
};
